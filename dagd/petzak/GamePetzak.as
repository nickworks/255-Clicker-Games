package dagd.petzak {
	...
	public class GamePetzak extends Game {
		...
		/*  
			...
			5 damage  - Jellyfish - Bounces up and down while slowly moving right
			10 damage - Squid - Slows down, does a spin, then speeds up
			20 damage - Shark - Eats oncoming friendly fish 
		*/
		...
		private function updateTime() {
			frames++;
			if (frames / 60 == Math.floor(frames / 60)) {
				time++;
				hud.timeboard.text = "Time: " + time;

				if (time / 30 == Math.floor(time / 30) && spawnMultiplier > 20) { // every 30 seconds
					spawnMultiplier -= 2; // increase spawn rate
				}
			}
		}
